...
const authenticateUser = async (req, res, next) => {
    ...
    if (!user) {
        return res.status(401).redirect("/login"); // Перенаправление на страницу входа при отсутствии аутентификации
    }
    req.user = user; // Сохраняем информацию о пользователе в объекте запроса для последующего использования
...
app.get("/list", async (req, res) => {
    try {
        ...
        let sortOptions = {};
        if (sort === 'team') {
            sortOptions = { Team: 1 }; // Sort by team name in ascending order
        } else if (sort === 'league') {
            sortOptions = { League: 1 }; // Sort by league in ascending order
        }
        ...
        // Get total count of teams based on the year filter
...
// Добавление маршрута для редактирования команды
